#!/usr/bin/env python3
"""Build R2 diagnostics for zone aggregation and tail-based steering."""
...
SUMMARY_OUT = ROOT / "summary_outputs"
FIGURE_OUT = ROOT / "figures"
...
def parse_city_year(weather: str) -> tuple[str, int]:
    parts = weather.split("_")
    return parts[0].title(), int(parts[-1])
...
def load_trace() -> pd.DataFrame:
    combined_candidates = [
        TRACE_DIR / "medium_office_control_traces.csv.gz",
        TRACE_DIR / "medium_office_control_traces.csv",
    ]
    sources = [path for path in combined_candidates if path.exists()]
    if not sources:
        sources = sorted(TRACE_DIR.glob("*_ordinal.csv.gz"))
    if not sources:
        sources = sorted(TRACE_DIR.glob("*_ordinal.csv"))
    if not sources:
        raise FileNotFoundError(
            f"No Medium Office trace files found in {TRACE_DIR}. "
            "Expected *_ordinal.csv.gz or medium_office_control_traces.csv(.gz)."
        )

    df = pd.concat((pd.read_csv(path) for path in sources), ignore_index=True)
    df = df[df["occupied"]].copy()
    df = df[df["discomfort_probability"].notna()].copy()
    city_year = df["weather"].map(parse_city_year)
    df["city"] = [x[0] for x in city_year]
    df["year"] = [x[1] for x in city_year]
    return df
...
def add_aggregation_fields(df: pd.DataFrame) -> tuple[pd.DataFrame, list[str], list[str], list[str]]:
    ...
    work["mean_gate_cat"] = gate_category(work["discomfort_probability"])
    work["max_zone_gate_cat"] = gate_category(work["zone_p_disc_max"])
    work["p90_zone_gate_cat"] = gate_category(work["zone_p_disc_p90"])
    work["gate_under_by_max"] = work["max_zone_gate_cat"] > work["mean_gate_cat"]
    work["gate_under_by_p90"] = work["p90_zone_gate_cat"] > work["mean_gate_cat"]
        ...
        rows.append(
            {
                ...
                "max_gate_more_conservative_pct": pct(sdf["gate_under_by_max"]),
                "p90_gate_more_conservative_pct": pct(sdf["gate_under_by_p90"]),
...
def summarize_steering_subsets(work: pd.DataFrame) -> pd.DataFrame:
    subsets = [
        ("all_occupied", work),
        ("gate_active_p_disc_gt_disc_low", work[work["discomfort_probability"].gt(DISC_LOW)]),
        (
            "moderate_gate_disc_low_to_disc_up",
            work[
                work["discomfort_probability"].gt(DISC_LOW)
                & work["discomfort_probability"].lt(DISC_UP)
            ],
        ),
        ("high_gate_p_disc_ge_disc_up", work[work["discomfort_probability"].ge(DISC_UP)]),
        (
...
def make_plot(summary: pd.DataFrame, work: pd.DataFrame) -> Path:
    city_year = summary[summary["year"] != "All"].copy()
    city_year["label"] = city_year["city"] + " " + city_year["year"].astype(str)
    ...
    out = FIGURE_OUT / "zone_aggregation_diagnostic.png"
...
def write_markdown(
    summary: pd.DataFrame, steering_summary: pd.DataFrame, zone_rank: pd.DataFrame
) -> Path:
    all_row = summary[(summary["city"] == "All") & (summary["year"] == "All")].iloc[0]
    city_rows = summary[summary["year"] == "All"].query("city != 'All'")
    steering_all = steering_summary[
        (steering_summary["city"] == "All")
        & steering_summary["subset"].isin(
            [
                "gate_active_p_disc_gt_disc_low",
                "high_gate_p_disc_ge_disc_up",
                "diagnostic_high_tail_p_disc_ge_020",
            ]
        )
    ]
    gate_city = steering_summary[
        (steering_summary["subset"] == "gate_active_p_disc_gt_disc_low")
        & (steering_summary["city"] != "All")
    ]
    path = SUMMARY_OUT / "zone_tail_diagnostic_summary.md"
    with path.open("w", encoding="utf-8") as f:
        f.write("# Zone Aggregation and Tail-Steering Diagnostic\n\n")
        f.write("Source traces: `traces/*_ordinal.csv.gz`\n\n")
        f.write("## Key Results\n\n")
        f.write(
        ...
        f.write(
            "- A max-zone gate would be more conservative than the submitted mean-zone gate in "
            f"{all_row.max_gate_more_conservative_pct:.1f}% of occupied probability timesteps; "
            f"a p90-zone gate would be more conservative in {all_row.p90_gate_more_conservative_pct:.1f}%.\n"
        )
        f.write(
            ...
            f"(n={int(all_row.mu_d_tail_sign_comparable_steps)}), supporting the current steering "
            "direction as a simple baseline while still motivating tail-based steering for coherence.\n"
        )
        f.write(
            "- Among gate-active states (`p_disc > 0.065`), the same sign conflict falls to "
            f"{steering_all.loc[steering_all['subset'] == 'gate_active_p_disc_gt_disc_low', 'mu_d_tail_sign_conflict_pct_of_comparable'].iloc[0]:.2f}% "
            "of comparable states; it is 0.00% for diagnostic high-tail states "
            "(`p_disc >= 0.20`) and for high-gate states (`p_disc >= 0.35`).\n"
        )
        f.write("\n## City-Level Summary\n\n")
        f.write(
            markdown_table(
                city_rows[
                    [
                        ...
                        "hidden_tail_020_pct",
                        "max_gate_more_conservative_pct",
        ...
        f.write(markdown_table(steering_all))
        f.write("\n\n### Gate-active by city\n\n")
        f.write(markdown_table(gate_city))
        f.write("\n\n## Highest-Risk Zones\n\n")
...
def main() -> int:
    ...
    steering_summary = summarize_steering_subsets(work)
    zone_rank = zone_ranking(work, p_cols)
    hidden_examples = write_top_hidden_examples(work)

    work_out = SUMMARY_OUT / "zone_aggregation_rows_compact.csv"
    cols = [
        "city",
        "year",
        "weather",
    ...
    summary.to_csv(SUMMARY_OUT / "zone_aggregation_summary.csv", index=False)
    steering_summary.to_csv(SUMMARY_OUT / "tail_steering_action_conditioned_summary.csv", index=False)
    zone_rank.to_csv(SUMMARY_OUT / "zone_risk_ranking.csv", index=False)
    hidden_examples.to_csv(SUMMARY_OUT / "hidden_zone_tail_examples.csv", index=False)
    make_plot(summary, work)
    write_markdown(summary, steering_summary, zone_rank)
    print(f"[diagnostic] wrote {SUMMARY_OUT} and {FIGURE_OUT}")
