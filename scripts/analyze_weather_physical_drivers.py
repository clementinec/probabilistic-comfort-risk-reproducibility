#!/usr/bin/env python3
"""Quantify mechanism-consistent associations in the simulation traces.
...
MANIFEST = ROOT / "data" / "panel_manifest.csv"
TRACE_DIR = ROOT / "restricted_inputs" / "simulation_traces"
PROVENANCE_DIR = ROOT / "outputs" / "weather" / "provenance"
OUT_LEGACY_DEFAULT = (
    ROOT / "outputs" / "weather" / "physical_drivers_legacy_timing"
)
OUT_CORRECTED_DEFAULT = ROOT / "outputs" / "weather" / "physical_drivers"
MODEL_DEFAULT = ROOT / "models" / "tsv_predictor_bundle.joblib"
ROBUSTNESS_HEADLINE = ROOT / "outputs" / "core" / "corrected_headline_case_summary.csv"
CORRECTED_NPZ_MANIFEST_DEFAULT = (
    ROOT / "outputs" / "core" / "input_trace_manifest.csv"
)
CORRECTED_NPZ_ROOT_DEFAULT = (
    ROOT / "outputs" / "core" / "corrected_zone_npz"
)
...
def load_corrected_bundle(path: Path):
    ...
    import legacy_control_pipeline as panel
...
These are associations in a fixed building and fixed operating configuration. They do
...
The available outputs support the physical interpretation reported here
without an EnergyPlus rerun. A rerun would be needed to make incident-solar,
envelope heat-flux, component-capacity, or unmet-load claims; those stronger
claims are outside this study's scope.
"""
