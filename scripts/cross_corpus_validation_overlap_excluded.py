...
and isotonic calibration match the deployed TSV prediction pipeline. Both the full
...
PANEL_SCRIPTS = REPO_ROOT / "scripts"
DEFAULT_DATA = (
    REPO_ROOT / "restricted_inputs" / "thermal_comfort" / "newin_with_bmr.csv"
)
DEFAULT_OUT = (
    REPO_ROOT / "outputs" / "validation" / "cross_corpus_overlap_excluded"
)
...
def write_summary(
    ...
    lines.extend(
        [
            ...
            "[0,.05,.10,.15,.20,.30,.40,.60,1.00], matching the primary validation.",
