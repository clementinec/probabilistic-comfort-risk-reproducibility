#!/usr/bin/env python
"""Probability-necessity diagnostic for comfort-risk control.

The diagnostic tests whether expected TSV is an insufficient control state by
measuring cases where the expected value looks neutral while the discomfort
tails remain large or directional.
"""
...
DEFAULT_TRACE = (
    ROOT
    / "rerun_medium_office_microgrid_ssp585"
    / "traces"
    / "medium_office_control_traces.csv"
)
DEFAULT_OUT = ROOT / "probability_necessity_diagnostic"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--strategies", nargs="+", default=["ordinal", "grid_gated"])
...
def build_threshold_grid(rows: pd.DataFrame, strategy: str = "ordinal") -> pd.DataFrame:
...
def add_city_year(rows: pd.DataFrame) -> pd.DataFrame:
    out = rows.copy()
    out["city"] = out["weather"].str.extract(r"^(beijing|houston|phoenix)", expand=False)
    out["city_label"] = out["city"].str.title()
    out["forecast_year"] = pd.to_numeric(
        out["weather"].str.extract(r"((?:19|20)\d{2})", expand=False),
        errors="coerce",
    )
    return out
...
def find_phoenix_matched_pair(rows: pd.DataFrame, mu_bin_width: float = 0.02) -> pd.DataFrame:
    sdf = add_city_year(rows[rows["strategy"] == "ordinal"]).reset_index(drop=True)
    sdf = sdf[
        sdf["city"].eq("phoenix")
        & sdf["forecast_year"].isin([2025, 2075])
    ...
    candidates = []
    for _, group in sdf.groupby("_mu_bin", sort=False):
        present = group[group["forecast_year"].eq(2025)]
        future = group[group["forecast_year"].eq(2075)]
...
def write_manuscript_figure(rows: pd.DataFrame, output_dir: Path) -> list[Path]:
    ordinal = add_city_year(rows[rows["strategy"] == "ordinal"].copy())
    if ordinal.empty:
        return []
    ...
    ax_scatter.set_title(
        "A. Expected sensation is informative but not sufficient near risk constraints",
        loc="left",
        weight="bold",
        fontsize=9.6,
    )
    ...
    if not pair.empty and len(pair) == 2:
        ...
        ax_pair.bar(x, cold, color="#4f6f9f", label=r"$P(\mathrm{TSV}\leq-2)$")
        ax_pair.bar(
            x,
            center,
            bottom=cold,
            color="#d9d9d9",
            label=r"$P(-1\leq\mathrm{TSV}\leq1)$",
        )
        ax_pair.bar(
            x,
            warm,
...
def case_label(stem: str) -> str:
    match = re.match(
        r"phoenix_ssp585_(near_2030s|mid_2050s|late_2080s)_(typical|hot|heatwave_extreme)_(\d{4})$",
        str(stem),
    )
    if not match:
        return str(stem).replace("_", " ")
    time_slice, severity, _year = match.groups()
    time_labels = {"near_2030s": "2030s", "mid_2050s": "2050s", "late_2080s": "2080s"}
    severity_labels = {
        "typical": "Typical",
        "hot": "Hot",
        "heatwave_extreme": "Heatwave",
    }
    return f"{time_labels[time_slice]} {severity_labels[severity]}"


def write_plots(rows: pd.DataFrame, examples: pd.DataFrame, output_dir: Path) -> list[Path]:
    paths = []
    ordinal = rows[rows["strategy"] == "ordinal"].copy()
...
def main() -> int:
    ...
    threshold_grid = build_threshold_grid(rows, strategy="ordinal")
    ...
    matched_mean_pairs = build_matched_mean_pair_diagnostics(
        rows,
        tail_threshold=args.tail_threshold,
        strategy="ordinal",
    )
    ...
    plot_paths = write_plots(rows, examples, args.output_dir)
    plot_paths.extend(write_manuscript_figure(rows, args.output_dir))
