#!/usr/bin/env python3
"""Endpoint-only and nominal-model robustness checks.

This script does not rerun EnergyPlus. It applies the ordinal and nominal
predictors already stored in this package to the same occupied zone-level
...
os.environ.setdefault("XDG_CACHE_HOME", str(REPO_ROOT / ".mplconfig"))
os.environ.setdefault("FC_CACHEDIR", str(REPO_ROOT / ".mplconfig"))
...
DEFAULT_MODEL = REPO_ROOT / "models" / "tsv_predictor_bundle.joblib"
DEFAULT_NO_PMV_MODEL = (
    REPO_ROOT / "models" / "tsv_predictor_bundle_no_pmv.joblib"
)
DEFAULT_DATA = (
    REPO_ROOT / "restricted_inputs" / "thermal_comfort" / "newin_with_bmr.csv"
)
DEFAULT_PANEL_MANIFEST = REPO_ROOT / "data" / "panel_manifest.csv"
DEFAULT_OUT = REPO_ROOT / "outputs" / "core"
...
def trace_paths(trace_dir: Path, max_cases: int | None) -> list[Path]:
    ...
    paths = [path for path in paths if path.name not in panel.COMBINED_TRACE_NAMES]
    if not paths:
...
def write_callback_timing_evidence(output_dir: Path) -> None:
    source = REPO_SCRIPTS / "legacy_control_pipeline.py"
    producing_source_sha256 = (
...
Distributed source SHA-256: `{file_sha256(source)}`

Producing source SHA-256: `{producing_source_sha256}`
