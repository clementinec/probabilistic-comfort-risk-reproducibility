#!/usr/bin/env python3
"""Build manuscript-facing artifacts from synchronized Paper A probability arrays.
...
* a legacy-to-corrected manuscript replacement map.
"""
...
DEFAULT_INPUT = ROOT / "paperA_R01/04_analysis/outputs/robustness_endpoint_model"
DEFAULT_OUTPUT = DEFAULT_INPUT / "manuscript_replacement"

os.environ.setdefault("MPLCONFIGDIR", str(ROOT / "paperA_R01/04_analysis/.mplconfig"))
os.environ.setdefault("XDG_CACHE_HOME", str(ROOT / "paperA_R01/04_analysis/.mplconfig"))
...
def manuscript_replacement_map(
    input_dir: Path,
    output_dir: Path,
    ...
    legacy_scalar_path = ROOT / "paperA_rebuild/diagnostics/scalar_tail_comparison/scalar_tail_global_summary.csv"
    legacy_scalar = pd.read_csv(legacy_scalar_path) if legacy_scalar_path.exists() else pd.DataFrame()
    old_unique_path = ROOT / "paperA_R01/04_analysis/outputs/unique_weather_sensitivity/global_comparison.csv"
    old_unique = pd.read_csv(old_unique_path) if old_unique_path.exists() else pd.DataFrame()
    legacy_zone_path = ROOT / "paperA_rebuild/diagnostics/zone_size_mosaic/zone_size_summary.csv"
    legacy_zone = pd.read_csv(legacy_zone_path) if legacy_zone_path.exists() else pd.DataFrame()
    legacy_floor_path = ROOT / "paperA_rebuild/diagnostics/zone_floor_comparison/zone_floor_summary.csv"
    ...
    old_mean_path = ROOT / "paperA_rebuild/diagnostics/mean_tail_compact/mean_tail_summary.csv"
    ...
    replacement.to_csv(output_dir / "manuscript_replacement_values.csv", index=False)
        ...
        [
            {
                ...
                "status": "REGENERATE",
                "reason": "Both axes are recomputed on the synchronized end-of-step environmental state.",
                "replacement": "figures/mean_vs_tail_scatter_sample_corrected.pdf",
            },
            {
                "object": "Figure 2: scalar-tail comparison",
                "status": "REGENERATE",
                "reason": "PMV, expected TSV, and p_tail are now all evaluated on the same recorded state.",
                "replacement": "figures/scalar_tail_comparison_hexbin_corrected.pdf",
            },
            {
                "object": "Figure 3: PMV-feature robustness",
                "status": "REGENERATE",
                "reason": "Corrected PMV and both ordinal outputs are synchronized and labels are explicit.",
                "replacement": "figures/pmv_feature_robustness_hexbin_corrected.pdf",
            },
            {
                "object": "Threshold-sensitivity figure",
                "status": "REGENERATE",
                "reason": "Use synchronized global_threshold_curves.csv; legacy directions can be checked but legacy coordinates should not be reused.",
                "replacement": "../robustness_threshold_curves.pdf",
            },
            {
                "object": "Zone-size mosaic figure",
                "status": "REGENERATE",
                "reason": "Every cell is recomputed from synchronized zone probabilities.",
                "replacement": "figures/zone_size_mosaic_heatmap_area.pdf",
            },
            {
                "object": "Zone/floor position figure",
                "status": "REGENERATE",
                "reason": "Zone, floor, warm-tail, and cold-tail summaries use synchronized probabilities.",
                "replacement": "figures/zone_floor_position_comparison.pdf",
            },
            {
                "object": "Held-out model calibration/validation figures",
                "status": "REMAINS VALID",
                "reason": "These use the held-out occupant dataset, not the callback-timed simulation trace.",
                "replacement": "None",
            },
            {
                "object": "Metabolic sensitivity figures",
                "status": "VERIFY/REGENERATE SEPARATELY",
                "reason": "Valid only if their inference was recomputed from recorded environmental fields; do not infer validity from the corrected reference-state run alone.",
                "replacement": "See metabolic-analysis provenance.",
            },
        ]
    )
    figure_status.to_csv(output_dir / "figure_regeneration_status.csv", index=False)
    ...
    lines = [
        ...
        "",
        "The full old-to-corrected ledger is `manuscript_replacement_values.csv`; it includes "
    ...
    (output_dir / "manuscript_replacement_map.md").write_text(
