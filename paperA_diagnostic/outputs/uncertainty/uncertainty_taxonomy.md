# Uncertainty taxonomy for Paper A R01
...
the EnergyPlus timestep. Those panel intervals are preserved in
`outputs/uncertainty_panel_legacy_timing/` for auditability and are not valid for
manuscript integration.

The corrected same-state robustness artifacts subsequently passed the 144-role,
119-unique-state schema, hash, finite-array, and count checks. The final panel
bootstraps are in `outputs/uncertainty_panel_corrected_same_state/` and
`outputs/uncertainty_panel_endpoint_corrected_same_state/`. Only these corrected
panel directories are eligible for manuscript integration. The independent
contributor-clustered held-out analyses are unaffected by trace timing.

## Purpose

The revision should not collapse all uncertainty into one confidence interval.
The existing artifacts support conditional resampling of cases and contributors,
but several larger uncertainties are structural and cannot be assigned empirical
coverage from this experiment. The table below separates them.

| Uncertainty source | What varies | Status in R01 | What the result can support | What it cannot support |
|---|---|---|---|---|
...
- Correcting the timestep alignment changes absolute broad-tail estimates by
  ...
  Legacy numbers nevertheless remain quarantined and must not be cited.
