...
This analysis repeats five contributor-disjoint train/calibration/test splits
with the same 400-tree ordinal LightGBM specification used by the study's
application model. A predicted tail event is defined consistently with the
manuscript as \(p_{\mathrm{tail}}\ge0.20\).

Run command:

```text
python scripts/grouped_tsv_validation_exact.py
```
